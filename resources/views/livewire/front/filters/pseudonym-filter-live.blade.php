<div class="w-full p-2 sm:p-6" x-data="{ expanded: true }">
        ...
        <div class="flex">
            ...
            @if (count($value))
                <button
                    ...
                    wire:click="$parent.resetFilter('pseudonyms', [])"
                    @click="$wire.search = ''"
                >
                    &times; Limpiar todos
                </button>
    ...
    @if (count($value))
        <div class="mt-4">
            @foreach ($value as $item)
                <span wire:key="pseudonym-selected-{{ $item }}" class="mb-1 inline-flex items-center px-2 py-1 me-2 text-sm font-medium capitalize text-blue-800 bg-blue-100 rounded dark:bg-blue-900 dark:text-blue-300">
                    {{ $this->pseudonym($item)?->name }}
                    <button wire:click="$parent.remove('pseudonyms', {{ $item }})" type="button" class="inline-flex items-center p-1 ms-2 text-sm text-blue-400 bg-transparent rounded-sm hover:bg-blue-200 hover:text-blue-900 dark:hover:bg-blue-800 dark:hover:text-blue-300" data-dismiss-target="#badge-dismiss-default" aria-label="Remove">
    ...
    <div class="mt-4" x-show="expanded" x-collapse>
        ...
        @if ($this->pseudonyms->count())
            <div class="max-h-52 overflow-y-auto space-y-1 px-2 mt-4">
                @foreach ($this->pseudonyms as $pseudonym)
                    <div wire:key="filters-pseudonym-{{ $pseudonym->id }}">
                        <label class="inline-flex items-center cursor-pointer">
                            <x-checkbox wire:model="value" value="{{ $pseudonym->id }}" />
